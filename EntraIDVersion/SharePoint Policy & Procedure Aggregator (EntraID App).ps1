...
function Load-EntraIDAppConfig {
    <#
    .SYNOPSIS
        Loads EntraID app configuration from a file and connects to SharePoint Online using PnP PowerShell with certificate authentication.
    #>
    param (
        [string]$ConfigFileName = "EntraID App Config.txt"
    )
    $ConfigFilePath = Join-Path -Path $PSScriptRoot -ChildPath $ConfigFileName
    
    if (-not (Join-Path  -Path $PSScriptRoot -ChildPath $ConfigFilePath)) {
        throw "Error: Configuration file not found at '$ConfigFilePath'. Please provide a valid file path."
    }
    ...
    try {
        Get-Content -Path $ConfigFilePath | ForEach-Object {
            ...
            if ($line -match "^ClientId\s*=\s*(.+)$") {
                $config.ClientId = $matches[1].Trim()
            }
            elseif ($line -match "^Tenant\s*=\s*(.+)$") {
                $config.Tenant = $matches[1].Trim()
            }
            elseif ($line -match "^PfxFileName\s*=\s*(.+)$") {
                $config.PfxFileName = $matches[1].Trim()
            }
        }
    }
    catch {
        throw "Error: Failed to read the configuration file. Ensure the file is properly formatted."
    }
    ...
    if ($missingValues.Count -gt 0) {
        throw "Error: Missing required configuration values: $($missingValues -join ', ')"
    }

    $resolvedPfxPath = Join-Path -Path $PSScriptRoot -ChildPath $config.PfxFileName
    if (-not (Test-Path -Path $resolvedPfxPath)) {
        throw "Error: Certificate file not found at '$resolvedPfxPath'. Ensure the file exists and is accessible."
    }
...
$entraIdAppConfigValues = Load-EntraIDAppConfig -ConfigFilePath $entraIdAppConfigPath
function Connect-ToSharePoint {
    ...
    try {
        ...
        Connect-PnPOnline -Url $SiteUrl -ClientId $entraIdAppConfigValues.ClientId -CertificatePath $entraIdAppConfigValues.PfxPath -Tenant $entraIdAppConfigValues.Tenant
